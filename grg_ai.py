...
HISTORY_FILE    = "./grg_history.json"  # persisted conversation log
TOP_K         = 5       # more chunks = more knowledge context
MAX_CHUNK     = 500     # larger chunks = more complete snippets
MIN_CHUNK     = 50
DISTANCE_THRESHOLD = 1.0
MAX_HISTORY_TURNS  = 40  # max exchanges kept on disk (older ones are dropped)
...
def _detect_gpu_layers() -> int:
    ...
    Install llama-cpp-python with GPU support for your hardware:
      NVIDIA CUDA (RTX 5060 / any GPU) — prebuilt wheel, works via PTX on Blackwell:
    ...
    if platform.system() == "Windows":
        try:
            ...
            # Prefer discrete GPU: NVIDIA (CUDA) > AMD (ROCm/Vulkan) > other
            nvidia = next(
                (g for g in gpus if any(k in g.upper() for k in ("NVIDIA", "GEFORCE", "RTX", "GTX", "QUADRO"))),
                None
            )
            amd = next(
                (g for g in gpus if any(k in g.upper() for k in ("AMD", "RADEON", "RX "))),
                None
            )
            if nvidia:
...
def _save_history(history: list[dict]) -> None:
    """Persist the last MAX_HISTORY_TURNS exchanges to disk."""
    try:
        with open(HISTORY_FILE, "w", encoding="utf-8") as f:
            json.dump(history[-MAX_HISTORY_TURNS:], f, ensure_ascii=False, indent=2)
    except Exception:
        pass
...
def main():
    ...
    def _load_llm(n_gpu_layers: int) -> "llama_cpp.Llama":
        kwargs = dict(
            model_path   = MODEL_PATH,
            n_ctx        = 4096,
            verbose      = False,
            n_threads    = cores,
            n_batch      = 512 if n_gpu_layers else 256,
            n_gpu_layers = n_gpu_layers,
            use_mmap     = True,
        )
        if n_gpu_layers:
            # flash_attn accelerates inference on RTX 30xx+ (Ampere and newer)
            try:
                return llama_cpp.Llama(**kwargs, flash_attn=True)
            except TypeError:
                pass  # older llama-cpp-python build — proceed without it
    ...
    print(f"--- Grg Ai ({'GPU' if gpu_layers else 'CPU'}) ---")

    conversation_history = _load_history()
    if conversation_history:
        print(f"[Loaded {len(conversation_history)} previous exchange(s). Type /clear to start fresh.]")
